...
logger = custom_log.setup_logging('metrics')
...
def show(model, deepmodel=False):
    ...
    if deepmodel:
        ...
        plot_roc_pr_curves(Y_test_tensor.cpu().numpy(),
                           outputs.cpu().numpy(),
                           predicted.cpu().numpy())
    # 预测测试集
    else:
...
def CNNx():
    ...
    model.load_state_dict(
        torch.load("models/CNN_04-29_1703_75.70%.pth", map_location=device))
...
def save_to_excel(metrics_list, filename):
    metrics_df = pd.DataFrame(metrics_list,
                              columns=[
                                  'Model', 'AUC', 'AUC CI_low', 'AUC CI_high',
...
def evaluate_imputation_strategy_metrics():
    # 读取数据集,验证每个数据集
    dataset_paths = utils.get_all_dataset()
    all_metrics_list = []
    for dataset in tqdm(dataset_paths):
        ...
        models_path = f'models/{method}'
        models_list = []
        # 遍历models文件夹中的所有文件
        logger.info(os.listdir(models_path))
        for filename in os.listdir(models_path):
            # 检查文件是否为.pkl文件
            if filename.endswith('.pkl'):
                # 构建完整的文件路径
        ...
        _, X_test, _, Y_test = utils.get_train_test_data(df)
        metrics_list = []
        for i, model in enumerate(models_list):
            metrics = calculate_metrics(model, X_test, Y_test)
            # 添加模型名称到指标列表
    ...
    # 保存所有评估结果到一个Excel文件
    save_to_excel(all_metrics_list, 'metrics_all.xlsx')
...
if __name__ == '__main__':
    # CNNx()
    # main()
    evaluate_imputation_strategy_metrics()
