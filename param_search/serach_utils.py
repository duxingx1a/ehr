...
def search_params(param_space, model, X_train, Y_train)-> None:
    """
    使用贝叶斯优化搜索超参数
    ...
    # 输出最佳参数
    print(f"Best parameters: ", opt_random_forest.best_params_)  # type: ignore
...
def get_train_test(df, name='roc_pr', train_size=0.8, random_state=42) -> List:
    ...
    - df: 输入的DataFrame，必须包含'have_stone'列作为标签
    - name: 数据集名称，用于标识数据集
